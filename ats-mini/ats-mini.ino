...
void setup()
{
  ...
  // 다음 전원 인가 시 리커버리 파티션에서 시작하도록 설정
  {
  ...
  // This slot booted successfully: confirm it so the bootloader keeps it.
  // The recovery uses OTA rollback to make "Sub" firmware a one-shot boot.
...
void loop()
{
  ...
  // Receive and execute web command
  int web_ev = web_event;
  web_event = 0;
  needRedraw |= !!(web_ev & REMOTE_CHANGED);
  pb1st.isPressed |= !!(web_ev & REMOTE_PRESSED);
  pb1st.wasClicked |= !!(web_ev & REMOTE_CLICK);
  pb1st.wasShortPressed |= !!(web_ev & REMOTE_SHORT_PRESS);
  int web_direction = web_ev >> REMOTE_DIRECTION;
  encCount = web_direction? web_direction : encCount;
  encCountAccel = web_direction? web_direction : encCountAccel;
  if(web_ev & REMOTE_PREFS) prefsRequestSave(SAVE_ALL);
